...
# Plot results
plt.title('4PL plots_vary_HillCoeff')
...
test_values = [ -1.0, 0.5, 1.0, 2.0, 8.0 ]
...
for i, (param, actual) in enumerate(zip('ACD', [A, C, D])):
    plt.text( 1.0E-4, 20.0 - i*6.0, '%s = %.3f' % (param, actual))

legend = plt.legend( test_values, loc='lower right')
legend.set_title('Hill Coeff')

plt.savefig( "4PL_vary_HillCoeff_semilog.png" )
plt.close()
