...
# load data
df = pd.read_csv('./data/AID_1460_datatable_week2_exercise.csv.gz', low_memory=False )
...
# max response must be at least 60%
...
df7.columns = ['PUBCHEM_CID'] + [ col + "_1460" for col in df7.columns.tolist()[1:] ]

# keep dataset for merging later (890 cpds)
df7.to_csv('AID_1460_cleaned_prioritized.csv', index=False )
